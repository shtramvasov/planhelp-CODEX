...
class Model {
    ...
    static async findOne(pginstance, {
        table = null,
        select = '*',
        joins = [],
        where = {},
        ...
        const result = await this.find(pginstance, {
            table,
            select,
            joins,
            where,
    ...
    static async findOneOrFail(pginstance, {
        table = null,
        select = '*',
        joins = [],
        where = {},
        ...
        const result = await this.find(pginstance, {
            ...
            order,
            group,
            limit: 2, // <-- лимит нужен, но минимум 2, чтобы понять есть ли строк > 2
            for_update,
        });

        if (result.length === 0 ) {
            throw Error(error_text || `Find return 0 rows`);
        }

        if (strict && result.length > 1) {
            throw Error(error_text || `Find return more 1 rows`);
        }
            ...
            if (keyWhere === '_custom') {
                ...
                for (let custom_where of customs){
                    if (custom_where.no_value){
                        ...
                        // mysql version
                        sql += ` ${custom_where.sql.replace(/\(:value:\)/g, `?`)} `;
                    }
                }
